...
const jsFuncMap = {
  ...
  'select': {
    "setup":(o,p)=>"",
    "loop":(o,p) => {let s = "(0"; for(let i=0; i < p.length-1; i++) s += `+((${p[i+1].loop})*((${i}==Math.floor(${p[0].loop}))? 1 : 0))`; return s+")";}},
  
  'toFreq' : {
    "setup":(o,p)=>"", 
    "loop":(o,p)=>`Math.pow(2, Math.floor(${p[0].loop}-69)/12) * 440`},
  ...
  // Oscillators and Basic signals
  // all oscillators have optional amplitude parameter <osc>(<frequency>, <amplitude=optional>);
  'sine': {
    "setup":(o,p)=>`${o} = new Module.maxiOsc(); 
      ${o}.phaseReset(${p.length>2 ? p[2].loop : 0.0});`, 
  ...
  'phasor2': {
    "setup":(o,p)=>`${o} = new Module.maxiOsc(); 
      ${o}.phaseReset(${p.length>1 ? p[1].loop : 0.0});`, 
    "loop":(o,p)=>`${o}.phasor(${p[0].loop}, (${p.length>3 ? p[2].loop : 0}), (${p.length>3 ? p[3].loop : 1}))`},
  ...
  'sqrt': {"setup":(o,p)=>"", "loop":(o,p)=>`Math.sqrt(${p[0].loop})`},
  'abs': {"setup":(o,p)=>"", "loop":(o,p)=>`Math.abs(${p[0].loop})`},
  'cos': {"setup":(o,p)=>"", "loop":(o,p)=>`Math.cos(${p[0].loop})`},
  'sin': {"setup":(o,p)=>"", "loop":(o,p)=>`Math.sin(${p[0].loop})`},

  // Amplitude/Gain (similar to mul, just more musical function name)
  'amp': {"setup":(o,p)=>"", "loop":(o,p)=>`(${p[0].loop} * ${p[1].loop})`},
  'gain': {"setup":(o,p)=>"", "loop":(o,p)=>`(${p[0].loop} * ${p[1].loop})`},

  // Envelope and sample&hold
  ...
  // arithmetic on list signal inputs
  ...
  // signal mapping and scaling functions
  ...
  'lores': {
    ...
    "loop":(o,p)=>`${o}.lores(${p[0].loop},${p.length>1 ? p[1].loop : 1000},${p.length>2 ? p[2].loop : 1})`},

  'hires': {
    "setup":(o,p)=>`${o} = new Module.maxiFilter()`, 
    "loop":(o,p)=>`${o}.hires(${p[0].loop},${p.length>1 ? p[1].loop : 1000},${p.length>2 ? p[2].loop : 1})`},
  
  // Input/Output, communication with JS editor, OSC, ADC
  //>>> deprecated
